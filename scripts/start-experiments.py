...
# Default Values
num_repetitions = 3
base_path = "/results_mode_all_test"

MISSING_FRACTIONS = [0.01, 0.1, 0.3, 0.5]
MISSING_TYPES = ["MCAR", "MNAR", "MAR"]
IMPUTER = ["mode"] #, "gain", "vae", "dl", "forest", "knn"] 
...
for task_id in task_ids:
    for imputer in IMPUTER:
        command = f" --set task_id={task_id} --set imputer={imputer} {name_arg} {types_arg} {strategies_arg} \
            {fractions_arg} {num_repetitions_arg} {base_path_arg}"
        output = subprocess.run(command, shell=True, capture_output=True)
        
        print(f"Started id: {task_id:<5} - imputer: {imputer:<6} ")
